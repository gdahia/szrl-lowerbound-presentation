...
\begin{frame}{Proof of Key Lemma}
  ...
  \begin{block}{Proof.}
    ...
    \pause
    %
    By the previous lemma, we have that $|H| \ge 5 \beta m/2$.
    %
    \pause
    %
    From $\Z$ $\beta$-refining $\X_r$ and averaging, there are \[(1 -
      \beta)k~\frac{5\beta}{4} > \beta k\] $Z_u$ such that $Z_u \subseteq_\beta X_h$ for
    $h \in H$.
    %
  \end{block}
  %
...
\begin{frame}{Proof of Key Lemma}
  ...
  \begin{proof}
    %
    Fix one such $(Z_u, X_h)$ and define
    %
    \vspace{-10pt}
    %
    \begin{multicols}{4}
